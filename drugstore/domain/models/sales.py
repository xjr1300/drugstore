...
class Sale:
    ...
    def add_sale_detail(self, sale_detail: SaleDetail) -> None:
        ...
        Raises:
            ValueError: 売上明細に記録されている売上IDが、追加する売上の売上IDと異なります。

        TODO: 次の単体テストを実装
        - 一般会員の顧客の売上の小計が3,000円未満の場合に、割引率が5%になり、小計、割引額、消費税額、合計が正しいことを確認
        - 一般会員の顧客の売上の小計が3,000円以上の場合に、割引率が10%になることを確認
        - 特別会員の顧客の売上の小計が3,000円未満の場合に、割引率が10%になることを確認
        - 特別会員の顧客の売上の小計が3,000円以上の場合に、割引率が20%になり、小計、割引額、消費税額、合計が正しいことを確認
        - 会員以外の顧客の売上の小計が3,000円未満の場合に、割引率が0%になることを確認
        - 会員以外の顧客の売上の小計が3,000円以上の場合に、割引率が0%になることを確認
        - 同じ商品の売上明細を売上に追加したときに、同じ商品の売上明細が追加されず、登録されている売上明細の数量と小計に足されていることを確認
        - 追加する売上明細の売上IDが、この売上の売上IDと異なる場合に例外をスローすることを確認
        """  # noqa: E501
